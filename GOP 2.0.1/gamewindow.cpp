...
QString text;
int deviation[4][2] = {{-20,-20},{-25,10},{10,-25},{0,0}};
    ...
    ui(new Ui::GameWindow)
{
    ...
    setWindowTitle("游戏界面");
    ui->playerInfo_0->setTitle("玩家A");
    ui->playerInfo_1->setTitle("玩家B");
    ui->playerInfo_2->setTitle("玩家C");
    ui->playerInfo_3->setTitle("玩家D");

    QPalette black;
    black.setColor(QPalette::WindowText,Qt::black);

    // 存档
    connect(ui->saveGame, &QPushButton::clicked, [=](){
        bd.saveboard();
        qDebug() << "save game successfully" << "\n";
        on_lineEdit_textChanged();
    });

    // 返回主界面按钮
    connect(ui->returnMainWindow, &QPushButton::clicked, [=](){
        this->close();
        emit GameWindow::openMainWidget();
    });

    for(int i = 0; i < 4; ++i) {
        player_image[i] = new QLabel(ui -> main);
        player_buy_image[i] = new QLabel(ui -> buy_interface);
        player_buy_image[i] -> move(180, 40);
    ...
    ui -> Mouse -> setPixmap(QPixmap(":/buy/buypic.png"));
    ui -> Mouse -> setScaledContents(true);
    ui -> map -> setCurrentIndex(0);
    ui -> move -> setIcon(QIcon(":/map/Buttons/move.png"));
    ui -> move -> setIconSize(size());
    ui -> buy -> setIcon(QIcon(":/map/Buttons/buy.png"));
    ui -> buy -> setIconSize(size());
    ui -> attack -> setIcon(QIcon(":/map/Buttons/attack.png"));
    ui -> attack -> setIconSize(size());
    ui -> mine -> setIcon(QIcon(":/map/Buttons/mine.png"));
    ui -> mine -> setIconSize(size());
    ui -> heal -> setIcon(QIcon(":/map/Buttons/heal.png"));
    ui -> heal -> setIconSize(size());
    ui -> tp -> setIcon(QIcon(":/map/Buttons/tp.png"));
    ui -> tp -> setIconSize(size());
    ui -> saveGame -> setIcon(QIcon(":/map/Buttons/save.png"));
    ui -> saveGame -> setIconSize(size());
    ui -> returnMainWindow -> setIcon(QIcon(":/map/Buttons/ingameexit.png"));
    ui -> returnMainWindow -> setIconSize(size());
    for (int i = 0; i < 6; ++i) {
    ...
    connect(ui->move, &QPushButton::clicked,[=](){
        ...
        ui -> returnMainWindow ->setEnabled(false);
    ...
    connect(ui->move_leave, &QPushButton::clicked,[=](){
        ...
        ui -> returnMainWindow ->setEnabled(true);
    ...
    for(int i = 1; i <= 5; ++i) {
        for (int j = 1; j <= 5; ++j) {
            connect(moveto[i][j], &QPushButton::clicked, [=](){
                ...
                on_lineEdit_textChanged();
                player_move_image[currPlayer] -> move(coordinate[bd.pl[currPlayer].pos[1] - 1]+deviation[currPlayer][0], coordinate[bd.pl[currPlayer].pos[0] - 1]+deviation[currPlayer][1]);
                player_image[currPlayer] -> move(coordinate[bd.pl[currPlayer].pos[1] - 1]+deviation[currPlayer][0], coordinate[bd.pl[currPlayer].pos[0] - 1]+deviation[currPlayer][1]);
...
void GameWindow::paintEvent(QPaintEvent *event)
{
    ...
    // 背景图片
    painter.drawPixmap(0,0,800,600,QPixmap(":/map/back.png"));
    painter.drawPixmap(0,150,450,450,QPixmap(":/map/GOPmap.png"));
    painter.drawPixmap(65,40,70,70,QPixmap(":/character1/idle/char0.png"));
    painter.drawPixmap(265,40,70,70,QPixmap(":/character2/idle/char1.png"));
    painter.drawPixmap(465,40,70,70,QPixmap(":/character3/idle/char2.png"));
    painter.drawPixmap(665,40,70,70,QPixmap(":/character4/idle/char3.png"));
...
void GameWindow::initMove(){
    ...
    refresh();
    moveto[1][3]->move(205, 10);
    for(int i = 2; i <= 4; ++i) {
        moveto[2][i]->move((i - 2) * 110 + 95, 95);
    }
    moveto[3][1] -> move(10, 205);
    moveto[3][5] -> move(400, 205);
    for (int i = 3; i <= 4; ++i) {
        for (int j = 2; j <= 4; ++j) {
            moveto[i][j]->move((j - 2) * 110 + 95, 95 + 110 * (i - 2));
        }
    }
    moveto[5][3] -> move(205, 400);
    for(int i = 0; i < 4; ++i) {
...
void GameWindow::initBuy(int shopsize){
    ...
    //ui -> buyPlayer -> setPalette(Qt::black);
    //ui -> curMoney -> setPalette(Qt::black);
    for(int i = 1; i <= 5; ++i) {
        buylist[i] -> setEnabled(true);
        //buylist[i] -> setPalette(Qt::black);
        //buylist[i] -> setStyleSheet("QPushButton{background-image: url(qrc:/buy/UI_1_mip_0.png;}");
        //buylist[i] ->setAutoFillBackground(true);
...
//                break;
    ...
    switch(bd.player_num) {
        case 2: player_image[2] -> hide();
            player_image[3] -> hide();
            player_image[0] -> move(coordinate[bd.pl[0].pos[1]-1]+deviation[currPlayer][0], coordinate[bd.pl[0].pos[0]-1]+deviation[currPlayer][1]);
            player_image[1] -> move(coordinate[bd.pl[1].pos[1]-1]+deviation[currPlayer][0], coordinate[bd.pl[1].pos[0]-1]+deviation[currPlayer][1]);
            player_move_image[0] -> move(coordinate[bd.pl[0].pos[1]-1]+deviation[currPlayer][0], coordinate[bd.pl[0].pos[0]-1]+deviation[currPlayer][1]);
            player_move_image[1] -> move(coordinate[bd.pl[1].pos[1]-1]+deviation[currPlayer][0], coordinate[bd.pl[1].pos[0]-1]+deviation[currPlayer][1]);
            player_image[0] -> show();
            player_image[1] -> show();
            break;
        case 3: player_image[3] -> hide();
            ...
            if (bd.pl[2].isAlive) {
                player_image[2] -> move(coordinate[bd.pl[2].pos[1]-1]+deviation[currPlayer][0], coordinate[bd.pl[2].pos[0]-1]+deviation[currPlayer][1]);
                player_move_image[2] -> move(coordinate[bd.pl[2].pos[1]-1]+deviation[currPlayer][0], coordinate[bd.pl[2].pos[0]-1]+deviation[currPlayer][1]);
                player_image[2] -> show();
            }
            break;
        default:
            ...
            if (bd.pl[3].isAlive) {
                player_image[3] -> move(coordinate[bd.pl[3].pos[1]-1]+deviation[currPlayer][0], coordinate[bd.pl[3].pos[0]-1]+deviation[currPlayer][1]);
                player_move_image[3] -> move(coordinate[bd.pl[3].pos[1]-1]+deviation[currPlayer][0], coordinate[bd.pl[3].pos[0]-1]+deviation[currPlayer][1]);
                player_image[3] -> show();
            }
            break;
    }
...
void GameWindow::tpTo_hm_0() {
    qDebug()<< "tp to home 0" << "\n";
    on_lineEdit_textChanged();
    player_move_image[currPlayer] -> move(coordinate[1]+deviation[currPlayer][0], coordinate[1]+deviation[currPlayer][1]);
    player_image[currPlayer] -> move(coordinate[1]+deviation[currPlayer][0], coordinate[1]+deviation[currPlayer][1]);
...
void GameWindow::tpTo_hm_1() {
    qDebug()<< "tp to home 1" << "\n";
    on_lineEdit_textChanged();
    player_move_image[currPlayer] -> move(coordinate[1]+deviation[currPlayer][0], coordinate[3]+deviation[currPlayer][1]);
    player_image[currPlayer] -> move(coordinate[1]+deviation[currPlayer][0], coordinate[3]+deviation[currPlayer][1]);
...
void GameWindow::tpTo_hm_2() {
    qDebug()<< "tp to home 2" << "\n";
    on_lineEdit_textChanged();
    player_move_image[currPlayer] -> move(coordinate[3]+deviation[currPlayer][0], coordinate[3]+deviation[currPlayer][1]);
    player_image[currPlayer] -> move(coordinate[3]+deviation[currPlayer][0], coordinate[3]+deviation[currPlayer][1]);
    bd.pl[currPlayer].pos[0] = 4;
...
void GameWindow::tpTo_hm_3() {
    qDebug()<< "tp to home 3" << "\n";
    on_lineEdit_textChanged();
    player_move_image[currPlayer] -> move(coordinate[3]+deviation[currPlayer][0], coordinate[1]+deviation[currPlayer][1]);
    player_image[currPlayer] -> move(coordinate[3]+deviation[currPlayer][0], coordinate[1]+deviation[currPlayer][1]);
    bd.pl[currPlayer].pos[0] = 2;
    bd.pl[currPlayer].pos[1] = 4;
    currPlayerNum[currPlayer] = 0;
    bd.pl[currPlayer].use_tp();
