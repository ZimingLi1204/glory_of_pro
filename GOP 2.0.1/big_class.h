...
const string playerName[4] = {"玩家A", "玩家B", "玩家C", "玩家D"};
...
public:
    ...
    void attack(player & target, const weapon & arm) {
        ...
        if (attackJudge(target, arm)) {
            int originHp = target.hp;
            // fist attack
            if (arm.type == 0) {
                fist().attack(target.hp);
                qDebug() << (char)('A' + id) << " attacked " << (char)('A' + target.id) << " with fist and cause " << originHp-target.hp << "damage\n";
            }
            // sword attack
            else if (arm.type == 1) {
                sword(arm.lv).attack(target.hp);
                qDebug() << (char)('A' + id) << " attacked " << (char)('A' + target.id) << " with sword lv" << arm.lv << " and cause " << originHp-target.hp << "damage\n";
            }
            // gun attack
            else if (arm.type == 2) {
                gun(arm.lv).attack(target.hp);
                qDebug() << (char)('A' + id) << " attacked " << (char)('A' + target.id) << " with gun lv" << arm.lv << " and cause " << originHp-target.hp << "damage\n";
            }
